...
(() => {
  ...
  function Plausible(defaults) {
    ...
    const enableAutoOutboundTracking = (targetNode = document, observerInit = {
      ...
      function trackClick(event) {
        trackEvent2("Outbound Link: Click", { props: { url: this.href } });
  ...
  // <stdin>
  var { trackPageview, trackEvent } = module_default({
    domain: "gethyas.com"
  });
  trackPageview();
...
;
window.plausible=window.plausible||function(){(window.plausible.q=window.plausible.q||[]).push(arguments)},document.querySelector("#deploy-to-netlify")!==null&&document.getElementById("deploy-to-netlify").addEventListener("click",function(){plausible("Deploy to Netlify")});const processForm=e=>{const t=new FormData(e);t.append("form-name","newsletter"),fetch("/",{method:"POST",body:t}).then(()=>{e.innerHTML='<p class="form--success mb-0">Thank you for subscribing!</p>'}).catch(t=>{e.innerHTML=`<p class="form--error mb-0">Oops. Something\\'s wrong: ${t}</p>`})},emailForm=document.querySelector(".email-form");if(emailForm&&emailForm.addEventListener("submit",e=>{e.preventDefault(),processForm(emailForm)}),document.querySelector("#sidebar-default")!==null){let e=document.getElementById("sidebar-default"),t=sessionStorage.getItem("sidebar-scroll");t!==null&&(e.scrollTop=parseInt(t,10)),window.addEventListener("beforeunload",()=>{sessionStorage.setItem("sidebar-scroll",e.scrollTop)})}